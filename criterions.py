...
class DiceLoss(nn.Module):
    ...
    def forward(self, preds, targets, weight=False):
        ...
        # den = num + torch.sum(FP, dim=(0)).float() + torch.sum(FN, dim=(0)).float()
...
def dce_evidence_u_loss_w_control(
    ...
    # ----------------------------------------
    # One hot label (must be float32)
    # ----------------------------------------
    label = F.one_hot(p, num_classes=c).float()
    label = label.view(-1, c)

    # ----------------------------------------
    # ACE Loss (Safe digamma)
    # ----------------------------------------
    ...
    # ----------------------------------------
    # Combine losses (ablation control)
    # ----------------------------------------
    total_loss = 0.0
